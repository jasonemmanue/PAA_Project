...
Cette page reproduit fidèlement le format des rapports officiels DEESP/DEEF du Port Autonome d'Abidjan : 17 tableaux et 12 graphiques générés automatiquement depuis les données collectées. Le Tableau 16 liste les tronçons codifiés congestionnés selon les règles officielles (un tronçon codifié est considéré congestionné s'il apparaît congestionné au moins 3 fois sur les 4 lundis du mois, ou 4 fois dans la semaine) ; les axes parents ne figurent pas dans ce tableau, seuls les tronçons codifiés (T1A, T1B, T1C…) y apparaissent. Le rapport est exportable en PDF (Tableau 16) et en Word (rapport complet).
