...
IMPORT EXCEL DANS LA MATRICE DES TEMPS. En mode écriture, un bouton "Importer Excel" apparaît dans la barre de la matrice des temps. Il accepte un fichier Excel (.xlsx, .xls) ou CSV avec 4 colonnes : date (ex. 2026-06-15), heure (0-23), troncon_id (identifiant du tronçon), duree_mn (durée en minutes décimales). Les données importées apparaissent aussitôt dans la matrice avec la mention "source : historique_paa_2025". Cela permet d'intégrer des mesures terrain ou des données historiques dans la visualisation sans passer par la console Railway.
