...
Cette page recense automatiquement toutes les 30 minutes les incidents signalés dans la zone portuaire d'Abidjan par les médias locaux : Fraternité Matin, Abidjan.net et Koaci. Seuls les articles mentionnant à la fois un type d'incident (accident, travaux, embouteillage…) ET un lieu de la zone portuaire (CARENA, Treichville, Palm Beach, pont HB, Boulevard de Marseille…) sont retenus — un article sur des travaux à Yakassé-Feyassé ou ailleurs hors zone est automatiquement écarté. Les incidents sont géolocalisés et affichés sur une carte avec des marqueurs colorés par sévérité. Les incidents actifs du dernier mois sont aussi visibles en superposition sur la carte principale. Un export CSV est disponible. Les filtres de type d'incident sont chargés dynamiquement depuis la base de données — ils reflètent toujours les types actifs configurés.
