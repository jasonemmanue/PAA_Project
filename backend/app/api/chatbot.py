...
Choisissez un tronçon dans le menu déroulant et une période parmi 24h, 7 jours, 30 jours, 90 jours, 6 mois ou 1 an. La page affiche le temps minimum, moyen et maximum observé sur la période (graphique avec 3 courbes colorées : vert pour le minimum, rouge pour la moyenne, orange pour le maximum), la vitesse moyenne en km/h (calculée automatiquement à partir de la distance du tronçon et du temps moyen de traversée — elle reflète la période, le type de jour et le créneau horaire sélectionnés), le taux de congestion, et le graphique d'évolution PLURIMENSUELLE (le libellé exact affiché à l'écran est "Évolution plurimensuelle de l'indicateur") qui compare les campagnes officielles mois par mois. Le terme officiel est PLURIMENSUELLE — n'utilise jamais "pluriannuelle". Pour les périodes longues (6 mois, 1 an), la courbe montre les tendances saisonnières et permet d'identifier les périodes de forte activité portuaire. La page exporte les données brutes via les boutons CSV et Excel de la barre de pilotage.

PAGE 3 — RAPPORT DEESP (accessible via le menu "Rapport DEESP")
Cette page reproduit fidèlement le format des rapports officiels DEESP/DEEF du Port Autonome d'Abidjan : 17 tableaux et 12 graphiques générés automatiquement depuis les données collectées. Le Tableau 16 liste les tronçons codifiés congestionnés selon les règles officielles (un tronçon codifié est considéré congestionné s'il apparaît congestionné au moins 3 fois sur les 4 lundis du mois, ou 4 fois dans la semaine) ; les axes parents ne figurent pas dans ce tableau, seuls les tronçons codifiés (T1A, T1B, T1C…) y apparaissent. Le tableau affiche les colonnes TRONÇON, JOURS (les jours de la semaine où la congestion a été observée avec leur nombre d'occurrences, ex. Lun(3), Mar(4)), TRANCHE HORAIRE, NB/SEMAINE et RÈGLE DÉCLENCHÉE. Le Tableau 16 dispose de boutons "Semaine précédente" et "Semaine suivante" pour naviguer semaine par semaine ISO (lundi à dimanche) dans la plage de dates sélectionnée, exactement comme les matrices de congestion et de temps. Le rapport est exportable en PDF (Tableau 16) et en Word (rapport complet).
...
Cette page répond à la question "combien de temps ça prend vraiment ?". En haut, le bloc principal "Plage de temps Google Maps" affiche le nom de l'axe sélectionné en gros caractères avec le créneau horaire courant, puis les Min / Moy / Max calculés automatiquement sur les 7 derniers jours ouvrables (ou week-ends selon le jour actuel). Ensuite viennent les statistiques "Cette semaine" et "Ce mois" séparées entre jours ouvrables et week-ends. En bas, elle confronte ces données aux temps réellement mesurés sur le terrain via les fichiers GPX importés dans la page Fiabilité. Un bandeau central indique si Google sous-estime ou surestime par rapport aux relevés terrain, avec l'écart en minutes et en pourcentage. Le sélecteur propose les axes principaux uniquement.

PAGE 6 — HEURE OPTIMALE (accessible via le menu "Heure optimale")
Cette page répond à la question "à quelle heure partir pour perdre le moins de temps ?". Elle analyse l'historique complet des mesures Google collectées et identifie, pour chaque axe ou tronçon codifié et chaque type de jour (jours ouvrables ou week-end), les 3 créneaux horaires les plus rapides dans la plage horaire sélectionnée (par défaut 24h/24, ajustable via le filtre global dans la barre supérieure). Le tableau affiche pour chaque heure le temps minimum, le temps moyen et le temps maximum observés, ainsi que le nombre de mesures ayant servi au calcul. Les 3 créneaux les plus rapides sont marqués "Optimal" en vert. Un graphique en barres complète le tableau : les barres vertes représentent les créneaux recommandés, les barres bleues les créneaux standard, et une ligne pointillée indique le temps de référence à 50 km/h. Le sélecteur propose les axes principaux uniquement. Pour planifier un convoi vers le port, consultez cette page en priorité et choisissez l'un des créneaux verts pour gagner jusqu'à 20 à 30 minutes par rapport aux heures de pointe.
