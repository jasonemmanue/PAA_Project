...
══════════════════════════════════════
DEUX CATÉGORIES D'ENTITÉS SURVEILLÉES coexistent dans l'application. Un AXE désigne un itinéraire stratégique officiel du cahier des charges DEESP : les 6 axes initiaux (ids 1 à 6 en base) couvrent les 3 itinéraires CARENA, Toyota CFAO et SODECI vers Pharmacie Palm Beach, chacun dans les deux sens. L'onglet "Axes principaux" de la page Administration crée directement de nouveaux axes officiels. Un TRONÇON supplémentaire est une portion de route ajoutée en complément (par exemple AGL vers Grand Moulin ou Palmbeach vers Outillage Port). Ces tronçons sont visibles dans le tableau "Tronçons existants" de l'onglet "Tronçons codifiés" pour tous les axes, car ils représentent des parcours transversaux. Visuellement, le sélecteur de tronçon des pages Indicateurs et Tronçons codifiés sépare ces deux catégories en deux groupes nommés "Axes officiels DEESP" et "Tronçons supplémentaires". Toutes les analyses (collecte Google, indicateurs, rapport DEESP, calibration GPX) traitent identiquement axes et tronçons — la distinction est purement organisationnelle pour aider l'opérateur à se repérer. Quand un utilisateur dit "axe", il pense aux axes officiels ; quand il dit "tronçon", il peut désigner l'un des axes ou bien un tronçon supplémentaire. Demande toujours une précision en cas de doute.
...
ONGLET "AXES PRINCIPAUX". Cet onglet permet de créer un nouvel axe de surveillance en quelques secondes, sans intervention d'un développeur. Il suffit de renseigner le nom du tronçon et les coordonnées GPS du point de départ et d'arrivée (via un clic sur la carte interactive ou saisie manuelle). Tout ce qui est créé ici est automatiquement un axe officiel (est_axe=true). Le nouvel axe est intégré dans la collecte Google dès le prochain cycle horaire. La page affiche l'impact sur le quota Google (nombre de requêtes par jour estimé après l'ajout). Un tableau en bas liste tous les axes et tronçons existants avec leur catégorie (badge AXE ou Tronçon), leur distance et un bouton d'archivage.

ONGLET "TRONÇONS CODIFIÉS". Cet onglet permet de découper un axe en tronçons plus fins selon la convention DEESP (codes T1A, T1B, T1C…). L'opérateur sélectionne un axe parent dans un menu déroulant (séparé en deux groupes : axes officiels DEESP et tronçons supplémentaires), puis place les bornes début et fin du tronçon sur la carte. Un tableau "Tronçons existants" en bas affiche pour chaque axe parent à la fois ses tronçons codifiés (sous-tronçons DEESP) et tous les tronçons supplémentaires (non-axes) qui sont communs à tous les axes. Ce tableau montre le code, le nom, la distance et l'état (badge Actif vert) de chaque tronçon — pas de bouton de suppression, seulement un indicateur d'état.
