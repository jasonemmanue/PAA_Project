...
Cette page recense automatiquement toutes les 30 minutes les incidents signalés dans la zone portuaire d'Abidjan par les médias locaux : Fraternité Matin, Abidjan.net et Koaci. Seuls les articles mentionnant à la fois un type d'incident (accident, travaux, embouteillage…) ET un lieu de la zone portuaire (CARENA, Treichville, Palm Beach, pont HB, Boulevard de Marseille…) sont retenus — un article sur des travaux à Yakassé-Feyassé ou ailleurs hors zone est automatiquement écarté. Les incidents sont géolocalisés et affichés sur une carte avec des marqueurs colorés par sévérité. Les incidents actifs de moins de 6 heures sont aussi visibles en superposition sur la carte principale. Un export CSV est disponible. Les filtres de type d'incident sont chargés dynamiquement depuis la base de données — ils reflètent toujours les types actifs configurés.

TYPES D'INCIDENTS CONFIGURABLES. En mode écriture, un panneau dépliable nommé Gérer les types d'incidents permet d'ajouter des catégories personnalisées, d'activer/désactiver les types existants, ou d'en supprimer. Chaque type est défini par un libellé affiché et une regex Python de détection (insensible à la casse). Le classificateur NLP utilise ces règles au prochain cycle d'enrichissement après chaque scraping. Le type "autre" est le fallback par défaut — il ne peut pas être supprimé. Exemples d'ajout : "Incendie / explosion" avec la regex incendie|explosion|feu de véhicule. Les 4 types de base (Accident, Route barrée, Travaux, Embouteillage) sont pré-configurés au déploiement et modifiables via ce panneau.
